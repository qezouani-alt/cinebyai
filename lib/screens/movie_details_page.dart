...
import '../widgets/full_watch_button.dart';
...
class _MovieDetailsPageState extends ConsumerState<MovieDetailsPage> {
  ...
  Future<void> _showInterstitialThenPop(Object? result) async {
    ...
    try {
      ...
      if (mounted && !_canPop) _isLeaving = false;
    }
  }
  ...
  void _showFullMovieUnlockDialog() {
    ...
    showCupertinoDialog<void>(
      ...
      builder: (dialogContext) => PremiumAccessDialog(
        title: 'Unlock Full Movie',
        message:
            'Watch a short ad to support our platform and watch this movie immediately.',
        onCancel: () => Navigator.of(dialogContext).pop(),
        onWatchAd: () async {
          Navigator.of(dialogContext).pop();
          // A missing or unfilled rewarded ad must not leave the primary watch
          // action unresponsive. When available, the ad is still presented
          ...
          Navigator.of(context).push(
            CupertinoPageRoute(
              builder: (context) =>
                  WatchScreen(movieId: movie.id, title: movie.title),
            ),
          );
        },
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        if (didPop) return;
        await _showInterstitialThenPop(result);
      },
                        ...
                        SafeArea(
                          child: Padding(
                            ...
                            child: CupertinoButton(
                              ...
                              onPressed: () async {
                                HapticFeedback.lightImpact();
                                await _showInterstitialThenPop(null);
                              },
                              child: Container(
                                width: 40,
                                height: 40,
  ...
  Widget _buildActionButton({
    ...
    if (isPrimary) {
      return FullWatchButton(icon: icon, label: label, onTap: onTap);
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(vertical: 14),
        decoration: BoxDecoration(
          color: AppTheme.darkSlate.withValues(alpha: 0.8),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: accentColor ?? CupertinoColors.white.withValues(alpha: 0.1),
            width: 1,
          ),
        ),
        child: Row(
