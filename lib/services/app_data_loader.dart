  ...
  Future<void> _loadFromCache() async {
    try {
      ...
      if (trendingJson != null) {
        final List<dynamic> list = json.decode(trendingJson);
        _trendingMovies = list.map((m) => Movie.fromJson(m)).toList();
      }

      // Load popular
      final popularJson = prefs.getString(_cacheKeyPopular);
      if (popularJson != null) {
        final List<dynamic> list = json.decode(popularJson);
        _popularMovies = list.map((m) => Movie.fromJson(m)).toList();
      }
    } catch (e) {
      if (kDebugMode) {
