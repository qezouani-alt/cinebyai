...
class AIService {
  ...
  Completer<List<Movie>>? _pendingMoodSearch;
  ...
  Future<String> _generateText(
    ...
    if (_apiKey.isEmpty) {
      throw StateError('Gemini is not configured for this build.');
    }
    if (_credentialsBlocked) {
      throw StateError('Gemini credentials are unavailable.');
    }
  ...
  Future<List<Movie>> searchByMood(
    ...
    final previousSearch = _pendingMoodSearch;
    if (previousSearch != null && !previousSearch.isCompleted) {
      // A new keystroke supersedes the previous request. Complete it rather
      // than leaving its caller waiting forever after the timer is cancelled.
      previousSearch.complete(const <Movie>[]);
    }

    // Create a completer for the debounced result
    final completer = Completer<List<Movie>>();
    _pendingMoodSearch = completer;
    ...
    _debounceTimer = Timer(debounceDuration, () async {
      try {
        ...
        if (predefinedMovies != null) {
          ...
          _completeMoodSearch(completer, predefinedMovies);
          return;
        }
...
''';
        ...
        try {
          ...
          _completeMoodSearch(completer, const <Movie>[]);
        }
      }
    });
  ...
  void _completeMoodSearch(
    Completer<List<Movie>> completer,
    List<Movie> movies,
  ) {
    if (!completer.isCompleted) completer.complete(movies);
    if (identical(_pendingMoodSearch, completer)) {
      _pendingMoodSearch = null;
    }
  }
